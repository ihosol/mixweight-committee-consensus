...
def main():
    ...
    ap.add_argument("--out", default=str(REPO / "dissertation_ua" / "papers" / "risk_budget_controller_2026_ua"
    ...
    for e in epochs:
        axL.axvspan(e - 0.5, e + 0.5, color=pcolor[phase[e]], alpha=0.07,
                    label={"SAFE": "БЕЗПЕЧНО", "ACTING": "ДІЄ", "VIOLATED": "ПОРУШЕНО"}[phase[e]] if phase[e] not in seen else None)
        seen.add(phase[e])

    ax_e = epochs
    axL.plot(ax_e, [B0[e] for e in ax_e], color="black", lw=2.0, ls="--",
             label=r"$B_t(0)$ некерована оцінка")
    if any(e in Blam for e in ax_e):
        axL.plot([e for e in ax_e if e in Blam], [Blam[e] for e in ax_e if e in Blam],
                 color="#d62728", lw=2.0, label=r"$B_t(\lambda_t)$ керована (увігнутий)")
    axL.axhline(eps, color="green", lw=1.3, ls=":", label=fr"допустимий рівень $\epsilon={eps}$")
    if alert is not None:
        axL.axvline(alert, color="#7f0000", lw=1.2)
        axL.annotate("червоний прапорець\n(насичення)", xy=(alert, eps), xytext=(alert + 1.0, eps - 0.22),
                     fontsize=8, color="#7f0000",
                     arrowprops=dict(arrowstyle="->", color="#7f0000", lw=1.0))
    axL.set_xlabel("епоха")
    axL.set_ylabel(fr"оцінка захоплення комітету ($\vartheta=1/3$)")
    axL.set_ylim(0, 1.0)
    ...
    if base_real:
        ...
        axR.plot(re, [base_real[e] for e in re], color="#555555", lw=1.2, marker="o",
                 ms=3, alpha=0.8, label=r"реалізоване $P(\geq\vartheta)$ без керування")
    axR.set_ylabel(r"реалізоване захоплення $P(\geq\vartheta)$, %")
    axR.set_ylim(0, 100)

    h1, l1 = axL.get_legend_handles_labels()
    h2, l2 = axR.get_legend_handles_labels()
    axL.legend(h1 + h2, l1 + l2, loc="center right", fontsize=7.5, framealpha=0.9)
    axL.set_title("Червоний прапорець: перехід оцінки за концентраційного дрейфу")
    fig.tight_layout()
    ...
    ax.plot(epochs, [headroom[e] for e in epochs], color="#1f77b4", lw=2.2,
            marker="o", ms=3, label=r"запас концентрації $\alpha_t^{\ast}-\alpha_t$")
    ax.plot(epochs, [gini[e] for e in epochs], color="#555555", lw=2.0, ls="--",
            label="Джині для стейку")
    ax.axhline(0.0, color="#1f77b4", lw=0.8, ls=":")
    if alert is not None:
        ax.axvline(alert, color="#7f0000", lw=1.2)
        ax.annotate(f"червоний прапорець: запас $=0$,\nДжині $={gini[alert]:.2f}$ (виглядає помірним)",
    ...
    ax.set_title("Запас концентрації зникає на червоному прапорці, а Джині лишається помірним")
    fig2.tight_layout()
